...
def load_data(data_root, dataset_name, feature, test_ratio, valid_ratio, warmup_ratio, rand_state,
    ...
    if dataset_name == 'Amazon':
        dataset = AmazonDataset(data_root=os.path.join(data_root, 'amazon_review_polarity_csv'))
        ...
        dataset = BiosDataset(data_root=os.path.join(data_root, 'iws_datasets'), filename='professor_teacher.csv')
    elif dataset_name == 'BiasBios-professor-physician':
        dataset = BiosDataset(data_root=os.path.join(data_root, 'iws_datasets'), filename='professor_physician.csv')
    elif dataset_name == 'BiasBios-journalist-photographer':
        dataset = BiosDataset(data_root=os.path.join(data_root, 'iws_datasets'), filename='journalist_photographer.csv')
    elif dataset_name == 'BiasBios-painter-architect':
        dataset = BiosDataset(data_root=os.path.join(data_root, 'iws_datasets'), filename='painter_architect.csv')
    ...
    if dataset_sample_size is not None and len(labels) > dataset_sample_size:
        # sample the dataset
        selected_indices = rand_state.choice(len(labels), size=dataset_sample_size, replace=False)
    ...
    count_vectorizer = CountVectorizer(strip_accents='ascii', binary=True)
    ...
    xs_mat_tr = count_vectorizer.transform(xs_text_tr)
    xs_mat_val = count_vectorizer.transform(xs_text_val)
    xs_mat_test = count_vectorizer.transform(xs_text_te)
    ...
    train_dataset = SentimentDataset(xs_text_tr, xs_token_tr, xs_feature_tr, ys_tr, xs_mat_tr,
                                     vocabulary=count_vectorizer.vocabulary_)
    valid_dataset = SentimentDataset(xs_text_val, xs_token_val, xs_feature_val, ys_val, xs_mat_val,
                                     vocabulary=count_vectorizer.vocabulary_)
    test_dataset = SentimentDataset(xs_text_te, xs_token_te, xs_feature_te, ys_te, xs_mat_test,
                                    vocabulary=count_vectorizer.vocabulary_)
...
class SentimentDataset:
    def __init__(self, xs_text, xs_token, xs_feature, ys, xs_mat, vocabulary):
        ...
        self.feature_names = [""] * len(self.vocabulary)
        for key in self.vocabulary:
            idx = self.vocabulary[key]
            self.feature_names[idx] = key
    ...
    def apply_lf(self, lf):
        if lf.keyword in self.vocabulary:
            kw_idx = self.vocabulary[lf.keyword]
            return self.xs[:, kw_idx].toarray().flatten() * lf.label
        else:
            return np.array(list(map(lf.apply, self.xs_text)))

    def get_feature_idx(self, feature_name):
        if feature_name in self.vocabulary:
            return self.vocabulary[feature_name]
        else:
            return -1

    def create_subset(self, indices):
        subset = SentimentDataset(self.xs_text[indices], self.xs_token[indices], self.xs_feature[indices],
                                  self.ys[indices], self.xs[indices, :], self.vocabulary)
        return subset
...
class BiosDataset:
    def __init__(self, data_root, filename):
        self.data_root = data_root
        self.raw_texts, self.labels = self.build_dataset(filename)
...
class YelpDataset:
    ...
    def build_dataset(self):
        train_file = os.path.join(self.data_root, 'train.csv')
        test_file = os.path.join(self.data_root, 'test.csv')
        df_train = pd.read_csv(train_file, sep=',', header=None)
        df_test = pd.read_csv(test_file, sep=',', header=None)
        df = pd.concat([df_train, df_test]).to_numpy()
        # subsample
