# This file is adapted from https://github.com/SamanthaBiegel/ActiveWeaSuL
...
class AWLabelModel:
    ...
    """
    def __init__(
        self, n_class=2, n_epochs: int = 200, lr: float = 1e-1, active_learning: bool = True,
            penalty_strength: float = 1, hide_progress_bar: bool = True):
        """Initialise model.
    ...
    def fit(self,
            L_train: np.ndarray,
            L_valid: Optional[np.ndarray] = None,
            y_valid: Optional[np.ndarray] = None,
            ground_truth_labels: Optional[np.array] = None):

        cliques = []  # assert conditional independence on LFs
        for i in range(L_train.shape[1]):
            cliques.append([i])

        if y_valid is None:
            # use the prediction result of majority vote to predict class balance
            mv = MajorityLabelVoter()
            y_valid = mv.predict(L_valid)

        y_valid = np.array(y_valid)
        class_balance = []
        for i in range(self.n_class):
            class_balance.append(float(np.mean(y_valid == i)))

        class_balance = np.array(class_balance)
        self._fit(L_train, cliques, class_balance, ground_truth_labels)
    ...
    def _predict(self, label_matrix=None, mu=None, P_Y=None, assign_train_labels=False):
    ...
    def predict_true(self, y_true, y_test=None, label_matrix=None):
        ...
        return self._predict(label_matrix, self.get_true_mu(y_true)[:, 1][:, None], self.E_S)
